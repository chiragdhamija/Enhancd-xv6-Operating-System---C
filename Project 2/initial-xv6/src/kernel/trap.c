...
int
pg_fault_handler(pagetable_t pagetable, uint64 va)        //CoWRef : references at end of report
{
    char *mem;
    pte_t *pte;
    if (va >= MAXVA)
...
void usertrap(void)
{
  ...
  if (r_scause() == 8)
  {
  ...
  else if (r_scause() == 15) {   //CoWRef : references at end of report
    ...
    if (va >= p->sz)
      p->killed = 1;
    // ret = pg_fault_handler(p->pagetable, va);
    if (pg_fault_handler(p->pagetable, va) != 0)
      p->killed = 1;
  } 
